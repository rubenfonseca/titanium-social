...
def generate_doc(config):
	...
	for file in os.listdir(docdir):
		if file in ignoreFiles or os.path.isdir(os.path.join(docdir, file)):
			continue
		md = codecs.open(os.path.join(docdir,file), mode="r", encoding="utf8").read()
		html = markdown.markdown(md)
		documentation.append({file:html});
	return documentation

def compile_js(manifest,config):
	js_file = os.path.join(cwd,'assets','com.0x82.social.js')
	...
	f = os.path.join(cwd,'Classes','Com0x82SocialModuleAssets.m')
	c = open(f).read()
...
ignoreFiles = ['.DS_Store','.gitignore','libTitanium.a','titanium.jar','README','com.0x82.social.js']
	...
	if docs!=None:
		for doc in docs:
			for file, html in doc.iteritems():
				...
				zf.writestr('%s/documentation/%s'%(modulepath,filename),html.encode('utf-8'))
