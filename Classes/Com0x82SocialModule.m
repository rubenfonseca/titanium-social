...
- (void)showActivityItems:(id)args
{
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    ENSURE_ARG_FOR_KEY(activityItems, options, @"activityItems", NSArray);
    NSArray *excludedActivityItems;
    ENSURE_ARG_OR_NIL_FOR_KEY(excludedActivityItems, options, @"excludedActivityTypes", NSArray);

    NSMutableDictionary *rect = [NSMutableDictionary dictionaryWithDictionary:@{ @"x" : @(0),
      @"y" : @(0),
      @"width" : @(0),
      @"height" : @(0) }];
    if (options[@"rect"]) {
    ...
    avc.completionWithItemsHandler = ^(UIActivityType _Nullable activityType, BOOL completed, NSArray *_Nullable returnedItems, NSError *_Nullable activityError) {
      [self fireEvent:@"activityWindowClosed"
           withObject:@{
             @"activityType" : activityType ?: [NSNull null],
             @"completed" : @(completed),
             @"error" : activityError != nil ? activityError.localizedDescription : [NSNull null]
           }];
    };
